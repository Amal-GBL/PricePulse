...
def scrape_blinkit_pepe():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 5-6) Focus search bar and type search term robustly
            print("Activating search bar and searching for term...")

            search_term = SEARCH_TERM
            search_anim_selectors = [
                "div.SearchBar__AnimationWrapper-sc-16lps2d-1",
                "div[role='search']",
            ]
            ...
            for anim_sel in search_anim_selectors:
                try:
                    page.wait_for_selector(anim_sel, state="visible", timeout=10000)
                    el = page.query_selector(anim_sel)
                    if el:
                        el.scroll_into_view_if_needed()
                        el.click()
                        time.sleep(0.5)
                        print(f"Clicked search animation wrapper using selector: {anim_sel}")
                        search_success = True
                        break
                except TimeoutError:
                    continue
                except Exception as e:
                    print(f"Error clicking {anim_sel}: {e}")

            if not search_success:
                print("Search wrapper not clickable; trying fallback to input field directly.")

            for input_sel in search_input_selectors:
                try:
                    page.wait_for_selector(input_sel, state="visible", timeout=8000)
                    ...
                    if inp:
                        ...
                        page.wait_for_load_state("networkidle", timeout=30000)
                        time.sleep(1)
                        print(f"Searched for '{search_term}' using input selector: {input_sel}")
                        search_success = True
                        break
                except TimeoutError:
                    continue
                except Exception as e:
                    print(f"Error typing in {input_sel}: {e}")

            if not search_success:
                print("Failed to find search bar. Navigating to fallback collection URL.")
